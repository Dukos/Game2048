...
public class BoardLogicCreator {
	...
	public CommandsListener createCommandListener(BoardArea area) {
		final BoardLineUtils utils = commandListenerProvider
				.createBoardLineUtils(area);

		final BoardLineMover mover = commandListenerProvider
				.createBoardLineMover(utils);
		final BoardLineMerger merger = commandListenerProvider
				.createBoardAreaMerger(area, utils);

		final MoveEntireBoardAlgorithm algorithm = commandListenerProvider
				.createMoveEntireBoardAlgorithm(mover, merger);
		final WallOrientationFactory factory = commandListenerProvider
				.createWallOrientationFactory(area.getDimensions());

		return commandListenerProvider
				.createCommandListener(algorithm, factory);
	}

	public ElementsAdder createElementsAdder(BoardArea area) {
		final Random random = elementsAdderProvider.createRandom();
		final Randomizer randomizer = elementsAdderProvider.createRandomizer(
				area, random);

		final FreeFieldFinder finder = elementsAdderProvider
				.createFreeFieldFinder(area);
