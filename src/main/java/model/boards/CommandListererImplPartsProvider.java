...
public class CommandListererImplPartsProvider {
	...
	public BoardLineMerger createBoardAreaMerger(BoardArea area,
			BoardLineUtils utils) {
		return new BoardLineMerger(area, utils);
	}

	public MoveEntireBoardAlgorithm createMoveEntireBoardAlgorithm(
			BoardLineMover mover, BoardLineMerger merger) {
		return new MoveEntireBoardAlgorithm(mover, merger);
	}

	public WallOrientationFactory createWallOrientationFactory(
			Dimension dimensions) {
		return new WallOrientationFactory(dimensions);
	}

	public CommandListenerImpl createCommandListener(
			MoveEntireBoardAlgorithm algorithm, WallOrientationFactory factory) {
