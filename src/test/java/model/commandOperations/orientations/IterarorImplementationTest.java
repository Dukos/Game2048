...
public class IterarorImplementationTest {
	...
	private final static Point LEFT_POSITION_CROSSED = new Point(-1, dimensions.height/2);
	private final static Point RIGHT_POSITION_CROSSED = new Point(dimensions.width, dimensions.height/2);
	private final static Point BOTTOM_POSITION_CROSSED = new Point(dimensions.width/2, dimensions.height);
	private final static Point UPPER_POSITION_CROSSED = new Point(dimensions.width/2, -1);
	...
	public IterarorImplementationTest(IterarorImplementation testObject, ExpectedResultsForPositions expectedResult)
	{
	...
	@Parameterized.Parameters
	   public static Collection<Object[]> createParameters() {
	      return Arrays.asList(new Object[][] {
	         { new ToRightIteratorImplementation(), new ExpectedResultsForPositions(true, false, true, true, new Point(4,2), new Point(0,2) )},
	         { new ToLeftIteratorImplementation(), new ExpectedResultsForPositions(false, true, true, true, new Point(2,2), new Point(5,2))},
	         { new ToBottomIteratorImplementation(), new ExpectedResultsForPositions(true, true, false, true, new Point(3,3), new Point(3,0))},
	         { new ToTopIteratorImplementation(), new ExpectedResultsForPositions(true, true, true, false, new Point(3,1), new Point(3,2))},
	      });
	   }
	
	@Test
	public void isIndexInRange_leftPositionCrossed_returnsExpectedValue() {
		final boolean result =  testObject.isIndexInRange(LEFT_POSITION_CROSSED, dimensions);
		assertEquals(result, expectedResults.left);
	}
	
	@Test
	public void isIdexInRange_rightPositionCrossed_returnsExpectedValue() {
		final boolean result =  testObject.isIndexInRange(RIGHT_POSITION_CROSSED, dimensions);
		assertEquals(result, expectedResults.right);
	}
	
	@Test
	public void isIndexInRange_upperPositionCrossed_returnsExpectedValue() {
		final boolean result =  testObject.isIndexInRange(UPPER_POSITION_CROSSED, dimensions);
		assertEquals(result, expectedResults.upper);
	}
	
	@Test
	public void isIndexInRange_bottomPositionCrossed_returnsExpectedValue() {
		final boolean result =  testObject.isIndexInRange(BOTTOM_POSITION_CROSSED, dimensions);
